...
var reDate = /^(\d{4}\-\d\d\-\d\d([tT][\d:\.]*)?)([zZ]|([+\-])(\d\d):(\d\d))?$/;
var reArray = /^(\n)?function Array\(\)/;
var reObject = /^(\n)?function Object\(\)/;
...
function convertJsonDates(value, index, parent) {
    ...
    if (typeof (value) == 'string' && reDate.test(value)) {
        if (parent)
            parent[index] = Date.fromISO(value);
        else
...
function Icon(name, tooltip) {
    if (name == null)
        return "";
    return "<span class='k-icon k-i-" + name + "' title = '" + tooltip + "'></span>";
}
...
function DateFormat(date, format) {
    return kendo.toString(makeDate(date), format);
}

function NotNullDateFormat(date, format) {
    return (date == null ? '' : kendo.toString(date, format));
}

function Error(message) {
    noty({text: message, position: 'topcenter', type: 'error', modal: true});
}